...
class ish_report(object):
  ...
  those methods available via magic methods '''
    ...
    MAP = {'AA1': ['LIQUID-PRECIP', 8, PrecipitationComponent],
           ...
           'KA1': ['EXTREME-AIR-TEMPERATURE', 10, ExtremeAirTemperature],
           'KA2': ['EXTREME-AIR-TEMPERATURE', 10, ExtremeAirTemperature],
           'KA3': ['EXTREME-AIR-TEMPERATURE', 10, ExtremeAirTemperature],
           'KA4': ['EXTREME-AIR-TEMPERATURE', 10, ExtremeAirTemperature],
           'KB1': ['AVERAGE-AIR-TEMPERATURE', 10],
           'KB2': ['AVERAGE-AIR-TEMPERATURE', 10],
           'KB3': ['AVERAGE-AIR-TEMPERATURE', 10],
...
Solar Irradiance: %s 
    ...
    def toJson(self):
        ...
        precipitation = []
        for i in range(1, 5):
            if self.get_additional_field(f"AA{i}"):
                precipitation.append(self.get_additional_field(f"AA{i}").toJson())
        if precipitation:
            dic_repport["liquid_precip"] = precipitation

        weather_occurence = []
        for i in range(1, 9):
            if self.get_additional_field(f"AU{i}"):
                weather_occurence.append(self.get_additional_field(f"AU{i}").toJson())
        if weather_occurence:
            dic_repport["weather_occurence"] = weather_occurence

        weather_condition = []
        for i in range(1, 6):
            if self.get_additional_field(f"AW{i}"):
                weather_condition.append(self.get_additional_field(f"AW{i}").toJson())
        if weather_condition:
            dic_repport["weather_condition"] = weather_condition

        sky_cover = []
        for i in range(1, 6):
            if self.get_additional_field(f"GA{i}"):
                sky_cover.append(self.get_additional_field(f"GA{i}").toJson())
        if sky_cover:
            dic_repport["sky_cover_condition"] = sky_cover

        extreme_temperature = []
        for i in range(1, 4):
            if self.get_additional_field(f"KA{i}"):
                extreme_temperature.append(self.get_additional_field(f"KA{i}").toJson())
        if extreme_temperature:
            dic_repport["extreme_temperature"] = extreme_temperature
    ...
    we have available for this code '''
        return self._additional.get(addl_code)
