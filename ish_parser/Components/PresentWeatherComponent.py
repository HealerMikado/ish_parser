...
class PresentWeatherComponent(BaseComponent):
    ...
    def loads(self, string):
        ...
        if self.INTENSITY[string[0:1]]:
            self.present_weather_array["intensity"] = self.INTENSITY[string[0:1]]
        if self.DESCRIPTOR[string[1:2]]:
            self.present_weather_array["descriptor"] = self.DESCRIPTOR[string[1:2]]
        if self.PRECIP[string[2:4]]:
            self.present_weather_array["precipitation"] = self.PRECIP[string[2:4]]
        if self.OBSCURATION[string[4:5]]:
            self.present_weather_array["obscuration"] = self.OBSCURATION[string[4:5]]
    ...
    def toJson(self):
        return self.present_weather_array
